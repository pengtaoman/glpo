...
type Member struct {
	...
	FooterData struct {
		...
		PartnerLink struct {
			Desc            string `json:"desc"`
			PartnerLinkList []struct {
				Link string `json:"link"`
				Logo string `json:"logo"`
				Name string `json:"name"`
			} `json:"partnerLinkList"`
		} `json:"partnerLink"`
...
func NilParser([]byte, string) ParseResult {
	fmt.Println("##################################### NilParser ############################")

	//var prs = ParseResult{}
	//for i := 0;  i < 10; i++ {
	//	url := "www.baidu.com?"+strconv.Itoa(i)
	//	item := "item000000000"+strconv.Itoa(i)
	//	req := Request{
	//		Url: url,
	//		Parser: NilParser,
	//	}
	//	prs.Requests = append(prs.Requests, req)
